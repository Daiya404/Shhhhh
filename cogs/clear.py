...
class Clear(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # This dictionary stores {channel_id: start_message_id} for the nuke command
    ...
    # --- Unique Prefix "Nuke" Command ---
    # This is a prefix command group that responds to `!tika nuke ...`
    @commands.group(invoke_without_command=True)
    async def nuke(self, ctx: commands.Context):
        """Base command for the two-step message nuke feature."""
        # This function runs if a user just types `!tika nuke`
        await ctx.send("To clear a range of messages, reply to the first message with `!tika nuke start`, then reply to the last message with `!tika nuke end`.", delete_after=15)
        try:
    ...
    @commands.has_permissions(manage_messages=True) # Basic permission check
    ...
    async def nuke_end(self, ctx: commands.Context):
        ...
        # Ensure chronological order by sorting the messages by creation time
        start_msg, end_msg = sorted([start_msg, end_msg], key=lambda m: m.created_at)

        # The API limit for bulk-deleting messages is 14 days.
        ...
        # Include the boundary messages if they are also recent enough
        ...
        try:
            ...
            await ctx.send(PERSONALITY["nuke_success"].format(count=len(messages_to_delete)), delete_after=10)
        except discord.Forbidden:
