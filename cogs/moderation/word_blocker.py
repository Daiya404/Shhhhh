...
class WordBlocker(commands.Cog):
    def __init__(self, bot):
        ...
        self.COOLDOWN_SECONDS = 1 # A slightly shorter cooldown is fine for moderation
    ...
    async def check_and_handle_message(self, message: discord.Message) -> bool:
        """Ultra-fast check that now captures the trigger word and respects cooldowns."""
        ...
        # Check against global blocklist first
        global_regex = guild_cache.get("global")
        if global_regex:
            match = global_regex.search(message.content)
            if match:
                triggered_word = match.group(1) # Capture the matched word

        # If not found in global, check user-specific list
        if not triggered_word:
            user_regex = guild_cache.get("users", {}).get(user_id)
            if user_regex:
                match = user_regex.search(message.content)
                if match:
                    triggered_word = match.group(1)

        # If a word was found, handle it
        if triggered_word:
    ...
    async def _handle_blocked_message(self, message: discord.Message, trigger_word: str):
        """Deletes the message and sends a more informative, rate-limited warning."""
        try:
            ...
            # If we've sent a warning in this channel recently, don't send another one.
            if now - last_warning_time < self.COOLDOWN_SECONDS:
                return
            
            # --- SEND CONTEXTUAL WARNING ---
            # Update the cooldown timestamp BEFORE sending the message
            self.channel_cooldowns[channel_id] = now
            # The original personality string didn't have a placeholder, so we add one.
    ...
    async def list_blockwords(self, interaction: discord.Interaction, user: Optional[discord.Member] = None):
        ...
        if user:
            words = guild_blocklist.get("users", {}).get(str(user.id), [])
            if not words:
                return await interaction.followup.send(self.personality["user_list_empty"].format(user=user.display_name))
            embed = discord.Embed(title=f"Blocked Words for {user.display_name}", description=", ".join(f"`{w}`" for w in words), color=discord.Color.orange())
        else:
