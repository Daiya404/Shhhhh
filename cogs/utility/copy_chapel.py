...
class CopyChapel(commands.Cog):
    ...
    def _get_config(self, guild_id: int) -> Optional[Dict]:
        ...
        return self.settings_cache.get(str(guild_id), {}).get("chapel_config")
    ...
    async def _handle_reaction(self, payload: discord.RawReactionActionEvent):
        ...
        if not config or str(payload.emoji) != config.get("emote"): return
        ...
        # This is the core logic: if count meets threshold, post/update. If not, delete.
    ...
    # --- Admin Command (Consolidated) ---
    @app_commands.command(name="chapel-admin", description="[Admin] Configure the message-copying chapel.")
    @app_commands.default_permissions(administrator=True)
    @is_bot_admin()
    @app_commands.describe(action="The action to perform.", channel="The channel for copied messages.", emote="The trigger emote.", threshold="Reactions needed.")
    @app_commands.choices(action=[app_commands.Choice(name="Configure", value="configure"), app_commands.Choice(name="View Status", value="status"), app_commands.Choice(name="Reset", value="reset")])
    async def chapel_admin(self, interaction: discord.Interaction, action: str, channel: Optional[discord.TextChannel] = None, emote: Optional[str] = None, threshold: Optional[app_commands.Range[int, 1, 100]] = None):
        await interaction.response.defer(ephemeral=True)
        guild_id = str(interaction.guild_id)
        if action == "configure":
            if not all([channel, emote, threshold]): return await interaction.followup.send("To configure, you must provide `channel`, `emote`, and `threshold`.")
            guild_settings = self.settings_cache.setdefault(guild_id, {}); guild_settings["chapel_config"] = {"channel_id": channel.id, "emote": emote.strip(), "threshold": threshold}
            await self.data_manager.save_data("role_settings", self.settings_cache)
            await interaction.followup.send(f"Done. Chapel is configured for {channel.mention} with {emote} and a threshold of **{threshold}**.")
        elif action == "status":
            config = self._get_config(interaction.guild_id)
            if not config: return await interaction.followup.send(self.personality["config_not_found"])
            ...
            embed.add_field(name="Channel", value=chapel_channel.mention if chapel_channel else "Not Found", inline=False); embed.add_field(name="Trigger Emote", value=config.get("emote", "Not Set"), inline=False); embed.add_field(name="Reaction Threshold", value=str(config.get("threshold", "Not Set")), inline=False)
            await interaction.followup.send(embed=embed)
        elif action == "reset":
            if self.settings_cache.get(guild_id, {}).pop("chapel_config", None):
                await self.data_manager.save_data("role_settings", self.settings_cache)
                await interaction.followup.send(self.personality["config_reset"])
            else:
                await interaction.followup.send(self.personality["config_not_found"])
    ...
    # --- YOUR ORIGINAL, WORKING EMBED LOGIC ---
    def _create_chapel_embed(self, message: discord.Message, emoji: str, count: int) -> discord.Embed:
        ...
        if message.attachments and (att := message.attachments[0]).content_type in ["image/jpeg", "image/png", "image/gif", "image/webp"]:
            image_url = att.url
        if message.reference and isinstance(message.reference.resolved, discord.Message):
            ...
            if not image_url and replied_to.attachments and (r_att := replied_to.attachments[0]).content_type in ["image/jpeg", "image/png", "image/gif", "image/webp"]:
                image_url = r_att.url
        ...
        embed.add_field(
            ...
            value=f"[{message.channel.mention}]({message.jump_url}) | {emoji} {count}"
        )
        return embed
