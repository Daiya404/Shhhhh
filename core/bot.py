...
class TikaBot(commands.Bot):
        ...
        try:
            ...
            if self.settings.GEMINI_API_KEY:
                try:
                    ...
                    self.logger.warning(f"AI services failed to initialize: {e}")
                    self.gemini_service = None
                    self.knowledge_service = None
            else:
                self.logger.warning("No Gemini API key found, AI features will be disabled")
    ...
    async def _handle_summarize_request(self, message: discord.Message):
        """Handle conversation summarization with personality and error handling."""
        feature_manager = self.get_cog("FeatureManager")
        ...
        # Check if summarize feature is enabled
        if feature_manager and not feature_manager.is_feature_enabled(message.guild.id, "ai_chat"):
            responses = [
                "The AI features are disabled here. Not my problem.",
                "I'm not allowed to think on this server. Take it up with the admins.",
                "AI is turned off here. Sorry, I guess."
            ]
            return await self._send_with_realistic_timing(
        ...
        try:
            ...
            network_responses = [
                "Network issues are preventing me from thinking clearly right now.",
                "My connection to my brain is spotty. Try again in a moment.",
                "The internet is being difficult. Give me a second."
            ]
            await self._send_with_realistic_timing(
                message.channel, random.choice(network_responses),
                reference_message=message, mention_author=False
            )
        except Exception as e:
    ...
    async def _handle_ai_conversation(self, message: discord.Message, is_mention: bool):
        """Handle AI conversation with improved personality, context, and error handling."""
        try:
            chat_cog = self.get_cog("AIChat")
            if not chat_cog or not self.gemini_service or not self.gemini_service.is_ready():
                fallback_responses = [
                    "My brain isn't working right now. Try talking to someone else.",
                    "I'm not feeling very chatty at the moment.",
                    "My AI is taking a coffee break. Sorry."
                ]
                return await self._send_with_realistic_timing(
                    message.channel, random.choice(fallback_responses),
                    reference_message=message, mention_author=False
                )
            ...
            network_error_responses = [
                "Sorry, I'm having network issues right now. Try again in a moment?",
                "My connection is being wonky. Give me a second to sort this out.",
                "Network gremlins are attacking. I'll be back shortly."
            ]
            await self._send_with_realistic_timing(
                message.channel, random.choice(network_error_responses),
                reference_message=message, mention_author=False
            )
        except Exception as e:
